...
func (p* ProcessThreads) Process(req stubs.Request, res *stubs.Response, p Params, c distributorChannels)(err error){
    if(req.Message == " "){
        err = errors.New("Message needed")
        return
        }

    res.Message = GolEngine(p , c )
              return
}

func (g* GameOfLife)GolEngine(p Params, c distributorChannels, req stubs.request, res *stubs.Response)(err error){
    if()
    c.ioCommand <- ioInput
